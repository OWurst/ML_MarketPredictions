...
#Add to List
stock_df_list <- list(appl_df, jnj_df, jpm_df, mrna_df, mtb_df, nvda_df, tgt_df, wmt_df, cost_df, pnc_df, abbv_df, meta_df)
```
...
full_learning_df <- make_full_learning_df(stock_df_list, 8)
head(full_learning_df)
```

```{r, save_PriceChangeByPrevious8Months_Training.csv}
write.csv("PriceChangeByPrevious8Months_Training.csv", file = file_path, row.names = FALSE)
```
