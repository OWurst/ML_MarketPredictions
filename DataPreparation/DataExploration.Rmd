...
```{r, make_overall_hist}
create_single_stock_histogram(combined_df, "Overall Monthly Percent Change Distribution", "1 Month Percent Change", FALSE)
```

```{r, make_overall_hist_adjusted}
create_single_stock_histogram(combined_df, "Overall Monthly Adjusted Percent Change Distribution", "1 Month Adjusted Percent Change", TRUE)
```
...
ggplot(combined_df, aes(x = Adj_Pct_Change, y = Volume)) +
  geom_point(shape = 3, size = .75, color = "blue") +
  facet_wrap(~Industry) +
  labs(title = "Monthly Transaction Above Average vs Adjusted Monthly Pct Change by Industry",
       y = "Number of Transactions",
       x = "Stock Price Adjusted Pct Change")
```
